...
```{r}
m1=lmer(CHI_MLU~Diagnosis+Gender+(1+SUBJ|Visit), data)
```
...
m0=lmer(CHI_MLU~Gender+(1+SUBJ|Visit), data)
anova(m1,m0)
...
```{r}
m2= lmer(CHI_MLU~Diagnosis+Gender+(1+SUBJ|Visit), data)

qwerty=lmer(CHI_MLU~Diagnosis+I(Visit^3)+(1+SUBJ|Visit),data,method="ML")
summary(qwerty)
r.squaredGLMM(qwerty)
