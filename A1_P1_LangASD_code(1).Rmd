...
```{r}
#eye balled the data, yes they are messy

#Change column names to match the other data sets
...
```{r}
#\\d is a regular expression meaning digit, so only the digits are extracted. This makes all visit into numbers
...
#------ Make subset of relevant variables ------
...
```{r}
#Fix the NA variables using the subsets created above
...
datamerge3 %>%
  filter(VISIT == 6) %>%
  arrange(desc(CHI_MLU)) #desc means descending, standard is ascending
