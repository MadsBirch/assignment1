...
dataDemo = read.csv("demo_train(1).csv")
dataLU = read.csv("LU_train.csv")
dataToken = read.csv("token_train.csv")
library(data.table)
library(ggplot2)
library(stringr)
library(tidyverse)
...
dataDemo= setnames(dataDemo,"Visit", "VISIT")

dataDemo= setnames(dataDemo, "Child.ID", "SUBJ")
...
dataDemo$VISIT= str_extract(dataDemo$VISIT, "\\d")
dataLU$VISIT= str_extract(dataLU$VISIT, "\\d")
dataToken$VISIT= str_extract(dataToken$VISIT, "\\d")
...
dataLU$SUBJ  = gsub("\\.", "", dataLU$SUBJ)
dataDemo$SUBJ  = gsub("\\.", "", dataDemo$SUBJ)
dataToken$SUBJ  = gsub("\\.", "", dataToken$SUBJ)
...
dataDemo= setnames(dataDemo, "MullenRaw", "nonverbalIQ")
dataDemo= setnames(dataDemo, "ExpressiveLangRaw", "verbalIQ")

demoSub = select(dataDemo, c("SUBJ", "VISIT", "Ethnicity", "Diagnosis", "Gender", "Age", "ADOS", "nonverbalIQ", "verbalIQ")) 

LUSub = select(dataLU, c("SUBJ", "VISIT", "MOT_MLU", "MOT_LUstd", "CHI_MLU", "CHI_LUstd"))

TokenSub = select(dataToken, c("SUBJ", "VISIT", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI"))
...
datamerge1 = merge(demoSub,LUSub,by= c("SUBJ", "VISIT"))
datamerge2 = merge(datamerge1, TokenSub, by=c("SUBJ", "VISIT"))
...
# rowSub = subset(datamerge2, VISIT==1, c("SUBJ", "ADOS", "nonverbalIQ", "verbalIQ"))

datamerge2$ADOS = na.locf(datamerge2$ADOS, na.rm= FALSE)
datamerge2$nonverbalIQ = na.locf(datamerge2$nonverbalIQ, na.rm= FALSE)
datamerge2$verbalIQ = na.locf(datamerge2$verbalIQ, na.rm= FALSE)
...
datamerge2$SUBJ = as.numeric(as.factor(datamerge2$SUBJ))

datamerge2$VISIT = as.numeric(datamerge2$VISIT)

datamerge2$Gender = mapvalues(datamerge2$Gender, c("1", "2"),c("F", "M"))
datamerge2$Diagnosis = mapvalues(datamerge2$Diagnosis, c("A", "B"),c("ASD", "TD"))
...
write.csv(datamerge2, file = "finaldata.csv", row.names = FALSE)
...
# filter 1
#filter = filter(datamerge2, datamerge2$CHI_MLU > 2.7)
...
#filter_2 = datamerge2[datamerge2$VISIT==1,]

#filter_2a=filter(filter_2,filter_2$CHI_MLU < 1.5)
...
Data = mutate(Data, GrandAvg = mean(tokens_CHI))

#datamerge3 = datamerge2 %>% 
 # mutate(MEAN_VISIT_MLU = by(datamerge2$CHI_MLU, datamerge2$VISIT, mean))
 
#by(datamerge2$CHI_MLU, datamerge2$VISIT, mean)
 
# 2

ChiAvg_AcrossVisits = Data %>%
  group_by(Child.ID)%>%
...
#datamerge2 = datamerge2 %>%
 # group_by(SUBJ) %>%
  #summarise(MEAN_CHILD_MLU = mean(CHI_MLU))
