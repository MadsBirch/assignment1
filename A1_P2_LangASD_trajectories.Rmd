...
Diagnosis did not significantly predict gender (b = 0.3 SE = 0.29, z = 1.12 , p > 0.05) or non-verbal IQ (beta = 0.9, se = 0.4887, t(350) = -1.83 p > 0.05) in the children. Therefore the two groups are evenly matched on these variables.

Diagnosis significantly predicted age (b = -12.7, SE = 0.87 ,t(344) = -14.41, p < 0.05). The mean age of the ASD children was 12.7 months older than that of the TD children, meaning that agewise, the two groups were not evenly matched.

In addition to age, diagnosis also significantly predicted verbal IQ (b = 2.79, SE = 0.66, t(350) = 4.21, p < 0.05). However, when plotting the verbal IQ, it becomes clear that ASD children are roughly divided into two groups. One group that scores higher than TD children and one that scores lower.
...
```{r}
m1=lmer(CHI_MLU~Diagnosis+Gender+VISIT+(1+VISIT|SUBJ), data)
```
...
m2= lmer(CHI_MLU~Diagnosis+Gender+VISIT+VISIT1+(1+VISIT|SUBJ), data)
...
m3 = lmer(CHI_MLU~Diagnosis+Gender+VISIT+VISIT1+VISIT2+(1+VISIT|SUBJ), data)
...
```{r}
#Plot of MOT_MLU, color signifies diagnosis
ggplot(data, aes(VISIT,MOT_MLU,color=Diagnosis,group=SUBJ))+geom_line()

#Model of hypothesis
m4=lmer(MOT_MLU~VISIT+Diagnosis+(1+VISIT|SUBJ),data)
